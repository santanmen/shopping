...
function generateRandomBarcode() {
    return 'BC' . str_pad(rand(0, 999999999), 9, '0', STR_PAD_LEFT);
}
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-sub-total item">Cargo de envío</th>
					<th class="cart-total last-item">Subtotal</th>
					<th class="cart-product-name item">Código de barras</th>
				...
				<tr>
					<td colspan="7">
						<div class="shopping-cart-btn">
							<span class="">
								<a href="index.php" class="btn btn-upper btn-primary outer-left-xs">Continua comprando</a>
...
<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "MXN"." ".$row['shippingCharge']; ?>.00</span></td>
					...
					<td class="cart-product-code">
    <?php echo generateRandomBarcode(); ?>
</td>
				</tr>
...
<div class="col-md-4 col-sm-12 cart-shopping-total">
    ...
    <tbody>
        <tr>
            <td>
                <div class="form-group">
                    <label for="paymentAmount">Cantidad con la que piensas pagar (opcional):</label>
                    <input type="number" class="form-control" id="paymentAmount" name="paymentAmount" placeholder="Ingresa la cantidad" oninput="calculateChange()">
                </div>
            </td>
        </tr>
        <tr>
            <td>
                <div id="changeAmount" class="cart-grand-total">
                    Cambio: <span class="inner-left-md">MXN 0.00</span>
                </div>
            </td>
        </tr>
        <tr>
            <td>
                <div class="cart-checkout-btn pull-right">
                    <button type="submit" name="ordersubmit" class="btn btn-primary">Proceder al pago</button>
                </div>
            </td>
        </tr>
    </tbody><!-- /tbody -->
...
<script type="text/javascript">
    function calculateChange() {
        ...
        if (paymentAmount) {
            if (changeAmount >= 0) {
                document.getElementById('changeAmount').innerHTML = 'Cambio: <span class="inner-left-md">MXN ' + changeAmount.toFixed(2) + '</span>';
            } else {
                document.getElementById('changeAmount').innerHTML = 'Cantidad insuficiente. Por favor, ingresa una cantidad mayor.';
            }
        } else {
            document.getElementById('changeAmount').innerHTML = 'Cambio: <span class="inner-left-md">MXN 0.00</span>';
        }
    }
</script>
...
<style>
	.cart-product-code {
    font-family: 'Libre Barcode 39', sans-serif; /* Asegúrate de que la fuente esté disponible */
    font-size: 24px; /* Ajusta el tamaño de la fuente según sea necesario */
    letter-spacing: 2px; /* Espaciado entre caracteres */
    text-align: center; /* Centra el texto dentro del td */
